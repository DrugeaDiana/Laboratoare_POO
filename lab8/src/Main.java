...
//        /* -------------- Sort using a lambda expression ------------ */
        copyStudents.sort((o1, o2) -> (int) (o2.getAverageGrade() - o1.getAverageGrade()));
...
//        /* Use the previously declared random object and use subjects.size() as your bound */
            for(Map.Entry<Student, LinkedList<String>> student : studentMap.entrySet()) {
                LinkedList<String> studentSubjects = new LinkedList<>();
                studentSubjects.push(subjects.get(random.nextInt(subjects.size())));
                studentSubjects.push(subjects.get(random.nextInt(subjects.size())));
                studentSubjects.push(subjects.get(random.nextInt(subjects.size())));
                studentSubjects.push(subjects.get(random.nextInt(subjects.size())));
                student.setValue(studentSubjects);
            }
